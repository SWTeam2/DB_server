...
def create_table_if_not_exists(conn, name):
    cur = conn.cursor()
    query = f"CREATE TABLE IF NOT EXISTS {name} (pred_id serial PRIMARY KEY, infer_time varchar, prediction float, timestamp varchar);"
    cur.execute(query)
    conn.commit()
    cur.close()
...
def insert_json_data(conn, table_name, json_data):
    ...
    infer_time = json_data["infer_time"]
    prediction = json_data["prediction"]
    timestamp = json_data["timestamp"]

    insert_sql = f'''
        INSERT INTO {table_name} (infer_time, prediction, timestamp) 
        VALUES (%s, %s, %s)
        RETURNING pred_id;
    '''
    cur.execute(insert_sql, (infer_time, prediction, timestamp))
